import type { WeaponType, WeaponsConfig } from '../simulation/types.ts';
import { getStashCash, getOwnedWeapons, addCashToStash, addWeapon } from '../persistence.ts';

export interface HubCallbacks {
  onStartRun: (weapon: WeaponType) => void;
  onBack: () => void;
}
...
export function setupHubScreen(callbacks: HubCallbacks, prices: ShopPrices, wc: WeaponsConfig): void {
  shopPrices = prices;
  weaponsConfig = wc;

  document.getElementById('hub-start-run')!.addEventListener('click', () => {
    callbacks.onStartRun(selectedWeapon);
  });
